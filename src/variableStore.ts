  ...
  getVar(varName: string, checkExistence: boolean = true): number | undefined {
    ...
    if (value === undefined && varName.includes(".")) {
      try {
        ...
        let current: any = (window as any).game || globalThis;
        ...
        throw new Error(
          `Failed to resolve variable "${varName}": ${(err as Error).message}`
        );
      }
    }
    ...
    for (const varName in json) {
      ...
      if (isEncodedNumber(varValue)) {
        ...
        if (varValue.length !== 3) {
          throw new Error(
            "Expect 3 elements: value, lower bound, upper bound."
          );
        }
