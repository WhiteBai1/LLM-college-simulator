...
import {
  ...
  AttributeRegistry,
  CombinedAttributeModifierAmounts,
} from "./attribute";
 ...
 *  ```
 */
export function loadEffectsFromObjectArray(
  objArray: any[],
  ...
  for (const obj of objArray) {
    ...
    for (const modifier of obj["attributeModifiers"]) {
      attributeModifiers.push(AttributeModifier.fromObject(modifier, registry));
    }
    result.push({
...
export class EffectProviderCollection<T extends EffectProvider>
  implements AttributeModifierSource
{
  ...
  onChanged?: (
    sender: EffectProviderCollection<T>,
    event: EffectProviderCollectionChangedEvent<T>
  ) => void;
  ...
  add(item: T | string, count: number = 1): void {
    ...
    if (this._items[item.id] == undefined) {
      ...
      this.dispatchChangeEvent(
        new EffectProviderCollectionChangedEvent(false, item, 0, count)
      );
    } else {
      ...
      this.dispatchChangeEvent(
        new EffectProviderCollectionChangedEvent(
          false,
          item,
  ...
  remove(item: T | string, count: number = 1): void {
    ...
    if (this._items[item.id][1] <= 0) {
      ...
      this.dispatchChangeEvent(
        new EffectProviderCollectionChangedEvent(false, item, 0, oldCount)
      );
    } else {
      this.dispatchChangeEvent(
        new EffectProviderCollectionChangedEvent(
  ...
  clear(): void {
    this.dispatchChangeEvent(
      new EffectProviderCollectionChangedEvent<T>(true, undefined, 0, 0)
    );
    this._items = {};
  }
  ...
  getCombinedAttributeModifierAmountsOf(
    attribute: Attribute
  ): CombinedAttributeModifierAmounts {
