...
export class GameEndEvent {
  constructor(
    public readonly state: EndGameState,
...
export class GameEngine {
  ...
  constructor(config: GameConfig, ap: GuiActionProxy) {
    ...
    this._actionFactory = new EventActionFactory(
  ...
  async loadGameData(): Promise<void> {
    ...
    const eventLoader = new GameEventLoader(
      this._expressionEngine,
      this._conditionFactory,
      this._actionFactory
    );
    if (this._config.eventDefinitionUrl) {
      ...
      console.log(`Successfully registered ${events.length} game events.`);
    } else {
  ...
  private _initFactories(): void {
    ...
    this._actionFactory.registerDeserializer(EASetItemAmounts);
    ...
    this._actionFactory.registerDeserializer({
      ID: EANPCChatAction.ID,
      fromJSONObject: EANPCChatAction.fromJSONObject,
    });
    ...
    this._actionFactory.registerDeserializer(EANPCChat as any);
  }
