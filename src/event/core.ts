...
import {CompiledEventExpression, EventExpressionEvaluator} from './expression';
...
export interface GuiActionProxy {
   ...
   */
  displayMessage(message: string, confirm: string, icon?: string, fx?: string):
      Promise<void>;
  /**
   ...
   */
  displayChoices(
      message: string, choices: Array<[string, number]>,
      icon?: string): Promise<number>;
}
 ...
 */
export interface EventActionExecutionContext extends
...
export abstract class EventAction {
   ...
   */
  abstract execute(context: EventActionExecutionContext): EventActionResult
...
export class EventActionList {
  constructor(
      private _actions: ReadonlyArray<EventAction>,
  ...
  execute(context: EventActionExecutionContext): EventActionResult
      |Promise<EventActionResult> {
  ...
  private _executeNext(context: EventActionExecutionContext): EventActionResult
      |Promise<EventActionResult> {
...
export class GameEvent {
  constructor(
      private _id: string, private _trigger: string,
      ...
      private _exclusions: string[] = [], private _once: boolean = false,
