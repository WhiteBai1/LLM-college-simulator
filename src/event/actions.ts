...
import {
  ...
  TranslationKeySourceFactory,
} from "./translationKeySource";
import { EANPCChat as EANPCChatImpl } from "./npcChat";
...
export class EventActionFactory {
  ...
  constructor(
    ...
    this._deserializationContext = {
      ...
      translationKeySourceFactory: new TranslationKeySourceFactory(
        conditionFactory,
        expressionCompiler
      ),
...
export class EADisplayMessage extends EventAction {
  constructor(
    private _message: TranslationKeySource,
...
export class EADisplayRandomMessage extends EventAction {
  constructor(
    private _messages: TranslationKeySource[],
    private _confirm: TranslationKeySource,
    private _icon: string
  ) {
    super();
...
export class EADisplayChoices extends EventAction {
  constructor(
    ...
    super();
    if (
      _choiceMessages.length !== _requirements.length ||
      _choiceMessages.length !== _actions.length
    ) {
      throw new Error(
        "The number of choices must be equal to the number of requirements/actions."
      );
    }
  }
  ...
  static fromJSONObject(
    ...
    if (obj["message"] == undefined) throw new Error("Message missing.");
    const message = context.translationKeySourceFactory.fromObject(
      obj["message"]
    );
    ...
    for (const c of obj["choices"]) {
      ...
      if (c["requirement"] != undefined) {
        requirements.push(context.expressionCompiler.compile(c["requirement"]));
      } else {
        requirements.push(context.expressionCompiler.compile("true"));
      }
    }
    return new EADisplayChoices(
      message,
      choiceMessages,
      requirements,
      actions,
  ...
  execute(
    ...
    for (let i = 0; i < this._choiceMessages.length; i++) {
      if (context.evaluator.eval(this._requirements[i])) {
        choices.push([this._choiceMessages[i].getTranslationKey(context), i]);
      }
    }
    return context.actionProxy
      .displayChoices(
...
export class EARandom extends EventAction {
  ...
  constructor(
    ...
    if (actions.length !== weightExprs.length) {
      throw new Error(
        "The number of weights must match the number of actions."
      );
    }
...
export class EACoinFlip extends EventAction {
  constructor(
    private _p: CompiledEventExpression,
  ...
  static fromJSONObject(
    ...
    let failActions = Array.isArray(obj["fail"])
      ...
      : [];
    return new EACoinFlip(
      context.expressionCompiler.compile(p),
...
export class EALoop extends EventAction {
  ...
  constructor(
    private _stopCondition: EventCondition | null,
  ...
  private _nextDoWhileIteration(
    ...
    do {
      ...
      if (typeof result === "number") {
        ...
        if (this._maxIterations > 0 && ++this._i >= this._maxIterations) {
          this._i = -1;
          break;
        }
      } else {
...
export class EAUpdateVariable extends EventAction {
  constructor(
    ...
    private _updateExpr: CompiledEventExpression
  ) {
    super();
  }
...
export class EAUpdateVariables extends EventAction {
  constructor(
    ...
    private _updateExprs: CompiledEventExpression[]
  ) {
    super();
    if (_varNames.length !== _updateExprs.length) {
      throw new Error(
        "The number of variables must be equal to the number of expressions."
      );
    }
  }
  ...
  static fromJSONObject(
    ...
    for (let varName in obj["updates"]) {
      ...
      exprs.push(context.expressionCompiler.compile(obj["updates"][varName]));
    }
...
export class EAGiveItem extends EventAction {
  constructor(
    ...
    private _amountExpr: CompiledEventExpression
  ) {
    super();
  }
  ...
  static fromJSONObject(
    ...
    return new EAGiveItem(
      obj["itemId"],
      context.expressionCompiler.compile(obj["amount"])
    );
  }
...
export class EATriggerEvents extends EventAction {
  ...
  execute(
    ...
    for (const trigger of this._triggerInfoList) {
      ...
      context.eventEngine.trigger(trigger.id, probability, trigger.priority);
    }
    return EventActionResult.Ok;
  }
}
...
export class EANPCChatAction /* adapter for npcChat */ {
  static ID = "EANPCChat";
  ...
  static fromJSONObject(
    ...
    if (obj["npcId"] == undefined) throw new Error("npcId missing.");
    if (typeof obj["npcId"] !== "number")
      throw new Error("npcId must be a number.");
    if (obj["prompt"] == undefined) throw new Error("prompt missing.");
    if (typeof obj["prompt"] !== "string")
      throw new Error("prompt must be a string.");
    if (obj["npcName"] == undefined) throw new Error("npcName missing.");
...
export class EAExamPrep extends EventAction {
  ...
  static fromJSONObject(
    ...
    if (obj["npcId"] == undefined)
      throw new Error("EAExamPrep: npcId missing.");
...
`.trim();
    ...
    try {
      ...
      const selected = await context.actionProxy.displayChoices(
        intro || `${npcName}：请选择你最想问的问题：`,
        choiceTuples,
        "OK"
      );

      const selIndex =
        typeof selected === "number" ? selected : Number(selected) || 0;
      const choosenText =
...
export class EAInterview extends EventAction {
  ...
  static fromJSONObject(
    ...
    if (obj["npcId"] == undefined)
      throw new Error("EAInterview: npcId missing.");
...
`.trim();
      ...
      if (typeof context.variableStore.setVar === "function") {
        ...
        console.warn(
          "EAInterview: variableStore has no setter API. score not saved:",
          newScore
        );
      }
