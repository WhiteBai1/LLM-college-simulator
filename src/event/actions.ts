...
export class EANPCChatAction /* adapter for npcChat */ {
    ...
    static fromJSONObject(obj: any, context: EventActionDeserializationContext): EventAction {
        ...
        if (typeof obj['npcId'] !== 'number') throw new Error('npcId must be a number.');
        if (obj['prompt'] == undefined) throw new Error('prompt missing.');
        if (typeof obj['prompt'] !== 'string') throw new Error('prompt must be a string.');
        if (obj['npcName'] == undefined) throw new Error('npcName missing.');
        if (typeof obj['npcName'] !== 'string') throw new Error('npcName must be a string.');
