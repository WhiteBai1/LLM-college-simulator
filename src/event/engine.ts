...
import {EventActionExecutionContext, EventActionResult, EventOccurrenceTracker, GameEvent} from './core';
...
export class GameEventEngine implements EventOccurrenceTracker {
  ...
  registerEvent(e: GameEvent): void {
    ...
    this._eventInfoById.set(
        e.id, {event: e, disabled: e.disabledByDefault, occurrenceCount: 0});
  ...
  unregisterEvent(e: GameEvent): void {
    ...
    throw new Error(
        'Invariance broken: event has info entry but not registered under any trigger id.');
  }
   ...
   */
  async processNextTrigger(context: EventActionExecutionContext):
      Promise<boolean> {
    ...
    if (this._ongoingTrigger != null) {
      throw new Error(
          `Cannot process the next trigger when the ongoing trigger "${
              this._ongoingTrigger}" is still being processed.`);
    }
  ...
  reset(): void {
    if (this._ongoingTrigger != null) {
      throw new Error(`Cannot reset the engine when the ongoing trigger "${
          this._ongoingTrigger}" is still being processed.`);
    }
    for (let info of this._eventInfoById.values()) {
