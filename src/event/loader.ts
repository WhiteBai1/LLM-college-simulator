...
export class GameEventLoader {
  ...
  constructor(
      ec: EventExpressionCompiler, cf: EventConditionFactory,
      af: EventActionFactory) {
  ...
  parseEvent(obj: any): GameEvent {
    ...
    const conditions = Array.isArray(obj['conditions']) ?
        obj['conditions'].map(
            (item: any) => this._conditionFactory.fromJSON(item)) :
        [];
    ...
    if (obj['probability'] == undefined) {
      ...
      if (typeof obj['probability'] === 'number') {
        ...
        throw new Error(
            'Probability must either a number or a string expression.');
      }
    }
    const exclusions =
        Array.isArray(obj['exclusions']) ? obj['exclusions'] : [];
    ...
    // 新增 event 的 priority
    const priority = typeof obj['priority'] === 'number' ? obj['priority'] : 0;

    return new GameEvent(
        id, trigger, conditions, actions, probability, exclusions, once,
        disabledByDefault, priority);
  }
}
